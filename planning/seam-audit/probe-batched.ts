...
export function tileToCrossSection(m: ManifoldToplevel, tile: Tile, opts: PipelineOptions = {}): CrossSection {
  ...
  console.log('STAGE', "let cs = parts.length", performance.now())
  ...
  console.log('STAGE', "if (opts.periodic) {", performance.now())
  if (opts.periodic) {
  ...
  console.log('STAGE', "if (opts.minFeature &&", performance.now())
  ...
  console.log('STAGE', "const lean =", performance.now())
  const lean = simplifyAnchored(cs.toPolygons() as Pt[][], tile.width, tile.height, 0.01)
  console.log('STAGE', "const simplified =", performance.now())
