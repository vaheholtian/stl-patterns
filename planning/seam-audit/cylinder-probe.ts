...
test('exported cylindrical cuts preserve real openings across the wrap at several detail settings', () => {
  ...
  try {
    ...
    for (const detail of [1, 2, 3]) {
      ...
      const simplified = cut.simplify(0); cut.delete()
      const data = writeBinaryStl(triMeshFromManifold(simplified)); simplified.delete()
      const reimported = manifoldFromTriMesh(m, extractSubMesh(parseStl(data), null))
      try {
        assert.equal(reimported.status(), 'NoError')
        const components=reimported.decompose(); console.log({components:components.map(c=>c.volume())}); components.forEach(c=>c.delete());
        let holes = 0, material = 0, mismatchCount = 0
        for (let i = 0; i < 400; i++) {
          ...
          if(sides[0]!==sides[1]) {mismatchCount++;if(mismatchCount<3)console.log(JSON.stringify({detail,z,hits:[-1,1].map(sign=>{const a=theta+sign*1e-5;return reimported.rayCast([0,0,z],[22*Math.cos(a),22*Math.sin(a),z]).map(h=>({p:h.position,n:h.normal}))})}))}
          if (sides[0]) material++; else holes++
        }
        console.log({detail,holes,material,mismatchCount}); assert.ok(holes > 100 && material > 100, 'seam must contain both openings and ribs')
