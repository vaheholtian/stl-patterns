...
export function selectByNormals(mesh: TriMesh, normals: [number, number, number][], where: (x: number, y: number, z: number) => boolean = () => true, tolDeg = 0.5): Uint32Array {
...
export function bentPlate(angleDeg: number): Body {
  ...
  // plate A leans right (rotated by -half), plate B leans left (+half); each starts past the bisector
  // and is trimmed at the mitre plane x = 0 (2026-09-09 fix: a plate starting at the ridge left a
  // V-shaped gap under a concave valley, so the plates only touched along the top line)
  ...
  return { name: `plate ${angleDeg}°`, body, mesh, region: selectByNormals(mesh, faceNormals, (x, y) => Math.hypot(x, y) < L - 1), faceNormals, origin }
}
